...
2. **"object of type 'map' has no len()"**
   - Usually indicates API version mismatch
   - Check method signatures and return types

3. **String encoding errors**
   - Always decode bytes properties: `track['title'].decode('utf-8')`

4. **Permission denied on save**
